...
func getTargetDate() time.Time {
	...
	return time.Date(2020, 11, 15, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2020, 11, 15, 15, 0, 0, 0, jst)
}
...
func main() {
	...
	var textTw string
	if near {
		text = "まもなく\n1st single「Don’t Blink」発売記念\nTalk Portトーク特典会！"
		textTw = "まもなく1st single 「Don’t Blink」発売記念Talk Portトーク特典会！"
		out = generateTodayImage(selectRandomImage(), text)
	} else {
		text = fmt.Sprintf("2020/11/15\n1st single「Don’t Blink」発売記念\nTalk Portトーク特典会まで\n%s!!", countdownText(now))
		textTw = fmt.Sprintf("2020/11/15 1st single「Don’t Blink」発売記念Talk Portトーク特典会まで%s!!", countdownText(now))
		out = generateTodayImage(selectRandomImage(), text)
	}
