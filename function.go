...
func getTargetDate() time.Time {
	...
	return time.Date(2020, 12, 13, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2020, 12, 13, 13, 0, 0, 0, jst)
}
...
func main() {
	...
	if near {
		text = "まもなく\n「Don’t Blink」\n発売記念インターネットサイン会！"
		textTw = "まもなく「Don’t Blink」 発売記念インターネットサイン会！"
		out = generateTodayImage(selectRandomImage(), text)
	} else {
		text = fmt.Sprintf("2020/12/13\n「Don’t Blink」\n発売記念インターネットサイン会まで\n%s!!", countdownText(now))
		textTw = fmt.Sprintf("2020/12/13 「Don’t Blink」 発売記念インターネットサイン会まで%s!!", countdownText(now))
		out = generateTodayImage(selectRandomImage(), text)
	}
