...
func getTargetDate() time.Time {
	...
	return time.Date(2019, 12, 24, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2019, 12, 24, 14, 50, 0, 0, jst)
}
...
func generateTodayImage(imgInfo ImageInfo, text string) image.Image {
	...
	for i, l := range lines {
		...
		dr := &font.Drawer{
			Dst:  out,
			Src:  image.NewUniform(color.RGBA{151, 95, 162, 255}),
			Face: face,
			Dot:  fixed.Point26_6{},
		}
