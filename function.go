...
func getTargetDate() time.Time {
	...
	return time.Date(2020, 9, 10, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2020, 9, 10, 18, 30, 0, 0, jst)
}
...
func main() {
	...
	if near {
		...
		text = fmt.Sprintf("2020/09/10\nニコSオンラインライブまで\n%s!!", countdownText(now))
		textTw = fmt.Sprintf("2020/09/10 ニコSオンラインライブまで%s!!", countdownText(now))
		out = generateTodayImage(selectRandomImage(), text)
	}
