...
func getTargetDate() time.Time {
	...
	return time.Date(2020, 9, 19, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2020, 9, 19, 17, 0, 0, 0, jst)
}
...
func main() {
	...
	var textTw string
	if near {
		text = "まもなく 「POP」発売記念\nTalkportオンラインリリースイベント!!"
		textTw = "まもなく 「POP」発売記念Talkportオンラインリリースイベント!!"
		out = generateTodayImage(selectRandomImage(), text)
	} else {
		text = fmt.Sprintf("2020/09/19\n「POP」発売記念\nTalkportオンラインリリースイベントまで\n%s!!", countdownText(now))
		textTw = fmt.Sprintf("2020/09/19 「POP」発売記念Talkportオンラインリリースイベントまで%s!!", countdownText(now))
		out = generateTodayImage(selectRandomImage(), text)
	}
