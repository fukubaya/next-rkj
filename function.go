...
func getTargetDate() time.Time {
	...
	return time.Date(2020, 12, 22, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2020, 12, 22, 15, 0, 0, 0, jst)
}
...
func main() {
	...
	if near {
		text = "まもなく\nLuna FESTIVAL！"
		textTw = "まもなくLuna FESTIVAL！"
		out = generateTodayImage(selectRandomImage(), text)
	} else {
		text = fmt.Sprintf("2020/12/22\nLuna FESTIVALまで\n%s!!", countdownText(now))
		textTw = fmt.Sprintf("2020/12/22 Luna FESTIVALまで%s!!", countdownText(now))
		out = generateTodayImage(selectRandomImage(), text)
	}
