...
func getTargetDate() time.Time {
	...
	return time.Date(2020, 12, 12, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2020, 12, 12, 12, 0, 0, 0, jst)
}
...
func main() {
	...
	if near {
		text = "まもなく\n「Don’t Blink」\n発売記念オンラインリリースイベント！"
		textTw = "まもなく「Don’t Blink」 発売記念オンラインリリースイベント！"
		out = generateTodayImage(selectRandomImage(), text)
	} else {
		text = fmt.Sprintf("2020/12/12\n「Don’t Blink」\n発売記念オンラインリリースイベントまで\n%s!!", countdownText(now))
		textTw = fmt.Sprintf("2020/12/12 「Don’t Blink」 発売記念オンラインリリースイベントまで%s!!", countdownText(now))
		out = generateTodayImage(selectRandomImage(), text)
	}
