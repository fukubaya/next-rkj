...
func getTargetDate() time.Time {
	...
	return time.Date(2020, 9, 6, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2020, 9, 6, 12, 0, 0, 0, jst)
}
...
func main() {
	...
	if near {
		...
		text = fmt.Sprintf("2020/09/06\n1stアルバム『POP』発売記念\nインターネットサイン会まで\n%s!!", countdownText(now))
		textTw = fmt.Sprintf("2020/09/06 1stアルバム『POP』発売記念 インターネットサイン会まで%s!!", countdownText(now))
		out = generateTodayImage(selectRandomImage(), text)
	}
