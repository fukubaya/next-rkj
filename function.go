...
func getTargetDate() time.Time {
	...
	return time.Date(2020, 10, 29, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2020, 10, 29, 20, 0, 0, 0, jst)
}
...
func main() {
	...
	if near {
		...
		text = fmt.Sprintf("2020/10/29\nニコSオンラインライブ再配信まで\n%s!!", countdownText(now))
		textTw = fmt.Sprintf("2020/10/29 ニコSオンラインライブ再配信まで%s!!", countdownText(now))
		out = generateTodayImage(selectRandomImage(), text)
	}
