...
func getTargetDate() time.Time {
	...
	return time.Date(2020, 12, 26, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2020, 12, 26, 17, 0, 0, 0, jst)
}
...
func main() {
	...
	if near {
		...
		textTw = "まもなく1st single 「Don’t Blink」 発売決定記念Talk Portトーク特典会！"
		out = generateTodayImage(selectRandomImage(), text)
	} else {
		text = fmt.Sprintf("2020/12/26\n1st single 「Don’t Blink」\n発売決定記念Talk Portトーク特典会まで\n%s!!", countdownText(now))
		textTw = fmt.Sprintf("2020/12/26 1st single 「Don’t Blink」 発売決定記念Talk Portトーク特典会まで%s!!", countdownText(now))
		out = generateTodayImage(selectRandomImage(), text)
	}
