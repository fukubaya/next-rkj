...
func getTargetDate() time.Time {
	...
	return time.Date(2020, 8, 12, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2020, 8, 12, 18, 0, 0, 0, jst)
}
...
func main() {
	...
	if near {
		...
		text = fmt.Sprintf("2020/08/12\nB.O.L.T.V 〜ボルティービー〜まで\n%s!!", countdownText(now))
		textTw = fmt.Sprintf("2020/08/12 までB.O.L.T.V 〜ボルティービー〜%s!!", countdownText(now))
		out = generateTodayImage(selectRandomImage(), text)
	}
