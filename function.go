...
func getTargetDate() time.Time {
	...
	return time.Date(2020, 10, 4, 0, 0, 0, 0, jst)
}

func getTargetDateTime() time.Time {
	jst, _ := time.LoadLocation(location)
	return time.Date(2020, 10, 4, 12, 55, 0, 0, jst)
}
...
func main() {
	...
	if near {
		...
		textTw = "まもなくTOKYO IDOL FESTIVAL オンライン2020!!"
		out = generateTodayImage(selectRandomImage(), text)
	} else {
		text = fmt.Sprintf("2020/10/04\nTOKYO IDOL FESTIVAL オンライン2020まで\n%s!!", countdownText(now))
		textTw = fmt.Sprintf("2020/10/04 TOKYO IDOL FESTIVAL オンライン2020まで%s!!", countdownText(now))
		out = generateTodayImage(selectRandomImage(), text)
	}
