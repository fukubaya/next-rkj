...
func TestDays(t *testing.T) {
	...
	// 当日0時の1秒前
	now := time.Date(2019, 12, 30, 23, 59, 59, 0, jst)
...
func TestDays2(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	// 当日0時の1n秒後
	now := time.Date(2019, 12, 31, 0, 0, 0, 1, jst)
	days := daysUntil(now, getTargetDate())
	if days != 0 {
		t.Errorf("days=%d", days)
	}
...
func TestHours(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	// 予定時刻の24時間30分前+1n秒後
	now := time.Date(2019, 12, 30, 17, 0, 0, 1, jst)
...
func TestHours2(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	// 予定時刻の23時間30分前
	now := time.Date(2019, 12, 30, 18, 0, 0, 0, jst)
	hours := hoursUntil(now, getTargetDateTime())
	if hours != 24 {
		t.Errorf("hours=%d", hours)
	}
...
func TestHours3(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	// 予定時刻の1時間前
	now := time.Date(2019, 12, 31, 16, 30, 0, 0, jst)
	hours := hoursUntil(now, getTargetDateTime())
	if hours != 1 {
		t.Errorf("hours=%d", hours)
	}
...
func TestNearTargetDateTime1(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	// 予定時刻の59分前
	now := time.Date(2019, 12, 31, 16, 29, 0, 0, jst)
...
func TestNearTargetDateTime2(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	// 予定時刻の1分前+1n秒後
	now := time.Date(2019, 12, 31, 17, 29, 0, 1, jst)
	near := nearTargetDateTime(now, getTargetDateTime())
	if !near {
		t.Errorf("near=%v", near)
	}
...
func TestNearTargetDateTime3(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	// 予定時刻の4分59秒後
	now := time.Date(2019, 12, 31, 17, 34, 59, 0, jst)
	near := nearTargetDateTime(now, getTargetDateTime())
	if !near {
		t.Errorf("near=%v", near)
	}
...
func TestNearTargetDateTime4(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	// 予定時刻の5分後
	now := time.Date(2019, 12, 31, 17, 35, 0, 0, jst)
	near := nearTargetDateTime(now, getTargetDateTime())
	if near {
		t.Errorf("near=%v", near)
	}
...
func TestCountdownText(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	// 予定時刻の100時間31分前
	now := time.Date(2019, 12, 27, 12, 59, 0, 0, jst)
...
func TestCountdownText2(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	// 予定時刻の100時間前
	now := time.Date(2019, 12, 27, 13, 30, 0, 0, jst)
	text := countdownText(now)
	if text != "あと 100 時間" {
		t.Errorf("text=%s", text)
	}
...
func TestGenerateImage(t *testing.T) {
	...
	for i, imgInfo := range imageList {
		t.Logf("%+v", imgInfo)
		out := generateTodayImage(imgInfo, "第3回 ももいろ歌合戦\n年の瀬芸能ショーのステージまで\nあと 18 日")
